...
wiki = Blueprint('appwiki', __name__, template_folder = 'templates', static_folder = 'static')

@wiki.before_app_request
def before_app_request():
		flask.g.database = get_database_object('mongodb')

#flask.g.database = get_database_object( 'mongodb' )

# Обсуждения
wiki.add_url_rule('/<word>/forum', methods = ['GET',], view_func = view_forum)
wiki.add_url_rule('/<word>/forum', methods = ['POST',], view_func = save_message)
...
# Вывести все статьи
wiki.add_url_rule('/all/<page_num>', methods = ['GET',], view_func = view_all_pages)
wiki.add_url_rule('/all', methods = ['GET',], view_func = view_all_pages)


wiki.add_url_rule('/alphabet/<letter>', methods = ['GET',], view_func = view_alphabet)
wiki.add_url_rule('/alphabet', methods = ['GET'], view_func = view_alphabet)

# Поиск по тегам
wiki.add_url_rule('/tags/<tags>', methods = ['GET',], view_func = view_find_tag)
...
wiki.add_url_rule('/<word>/edit', methods = ['GET',], view_func = get_form_edit)
# Правка статьи
wiki.add_url_rule('/<word>/edit', methods = ['POST',], view_func = save_edit)

# Получить форму для истории
wiki.add_url_rule('/<word>/history', methods = ['GET',], view_func = get_form_history)
# Правка истории
wiki.add_url_rule('/<word>/history', methods = ['POST',], view_func = save_history)
# Показ отдельного поста из истории
wiki.add_url_rule('/<word>/history/<id>', methods = ['GET',], view_func = get_history_page)

# Выборка избранных статей
wiki.add_url_rule('/favorites/<num>', methods = ['GET',], view_func = get_pages_favorites)
wiki.add_url_rule('/favorites', methods = ['GET',], view_func = get_pages_favorites)

# Выборка статей для которых проводились правки
wiki.add_url_rule('/latedit/<num>', methods = ['GET',], view_func = get_pages_latedit)
wiki.add_url_rule('/latedit', methods = ['GET',], view_func = get_pages_latedit)

# Выборка последних статей
wiki.add_url_rule('/latter/<num>', methods = ['GET',], view_func = get_pages_latter)
